    ...
    <header class="bg-white border-b border-slate-200 px-4 md:px-8 py-4">
        <div class="max-w-6xl mx-auto flex items-center justify-between">
            <div class="flex items-center gap-4">
                <a href="{{ route('dashboard.user') }}" class="p-2 hover:bg-slate-100 rounded-xl transition-all">
                ...
                <div>
                    ...
                    <p class="text-sm text-slate-600">{{ $books->total() }} Buku Tersedia</p>
                </div>
            </div>
            <div class="flex items-center gap-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-12">
                    @forelse ($books as $book)
                        <div class="group relative bg-white rounded-3xl shadow-lg border border-slate-100 overflow-hidden hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 hover:border-indigo-200">
                            ...
                            <div class="p-6">
                                <div class="mb-3">
                                    <span class="inline-flex px-2.5 py-1 bg-indigo-100 text-indigo-700 text-xs font-semibold rounded-full uppercase tracking-wide">{{ $book->kategori }}</span>
                                </div>
                                <h3 class="font-bold text-lg leading-tight line-clamp-2 text-slate-800 group-hover:text-indigo-700 mb-2 transition-colors">{{ $book->judul }}</h3>
                                <p class="text-slate-600 font-medium mb-1">{{ $book->penulis }}</p>
                                <p class="text-sm text-slate-500 mb-4">{{ $book->penerbit ?? 'Independen' }} • {{ $book->tahun_terbit }}</p>

                                {{-- Action button --}}
                                <div class="flex items-center justify-between">
                                    <span class="text-xs text-slate-400 font-medium">Stok {{ $book->stok }}</span>
                    ...
                    @empty
                        <div class="col-span-full text-center py-24">
                            ...
                            <a href="{{ route('dashboard.user') }}" class="inline-flex items-center gap-2 px-6 py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold rounded-2xl shadow-lg hover:shadow-xl transition-all">
