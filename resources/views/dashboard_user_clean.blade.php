...
@section('page-description', 'Tampilan pengguna yang ringkas, informatif, dan mudah dipahami.')

@section('content')
    <div class="space-y-6">
        @include('components.app-banner')
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5">
            <div class="rounded-3xl bg-white p-6 shadow-lg shadow-slate-200/60 border border-slate-100">
                ...
                <div class="mt-3 flex items-end justify-between">
                    <span class="text-3xl font-black text-slate-900">{{ number_format($stats['bookCount'] ?? 0) }}</span>
            ...
            <div class="rounded-3xl bg-white p-6 shadow-lg shadow-slate-200/60 border border-slate-100">
                ...
                <div class="mt-3 flex items-end justify-between">
                    <span class="text-3xl font-black text-slate-900">{{ number_format($stats['stockCount'] ?? 0) }}</span>
            ...
            <div class="rounded-3xl bg-white p-6 shadow-lg shadow-slate-200/60 border border-slate-100">
                ...
                <div class="mt-3 flex items-end justify-between">
                    <span class="text-3xl font-black text-slate-900">{{ number_format($stats['availableCount'] ?? 0) }}</span>
            ...
            <div class="rounded-3xl bg-white p-6 shadow-lg shadow-slate-200/60 border border-slate-100">
                ...
                <div class="mt-3 flex items-end justify-between">
                    <span class="text-3xl font-black text-slate-900">{{ number_format($stats['categoryCount'] ?? 0) }}</span>
        ...
        <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
            <div class="xl:col-span-2 rounded-[2rem] bg-white border border-slate-100 shadow-xl shadow-slate-200/60 p-6 sm:p-8">
                <div class="flex items-center justify-between gap-4 mb-6">
                    ...
                    <a href="{{ route('user.books') }}" class="text-sm font-semibold text-blue-700 hover:text-blue-800">Lihat semua</a>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    @forelse ($featuredBooks as $book)
                        <div class="rounded-3xl border border-slate-100 bg-slate-50/80 p-4 flex gap-4">
                            ...
                            <div class="min-w-0">
                                <span class="inline-flex px-2.5 py-1 rounded-full bg-blue-100 text-blue-700 text-[10px] font-bold uppercase tracking-[0.18em]">{{ $book->kategori }}</span>
                                <h5 class="mt-3 text-base font-bold text-slate-900 leading-snug">{{ $book->judul }}</h5>
                                <p class="mt-1 text-sm text-slate-500">{{ $book->penulis }}</p>
                                <p class="mt-4 text-xs text-slate-400">{{ $book->penerbit ?? 'Penerbit tidak tersedia' }} • {{ $book->tahun_terbit }}</p>
                            </div>
                        </div>
                    @empty
            ...
            <div class="rounded-[2rem] bg-white border border-slate-100 shadow-xl shadow-slate-200/60 p-6 sm:p-8">
                ...
                <div class="space-y-3">
                    <a href="{{ route('user.books') }}" class="flex items-center gap-3 rounded-2xl border border-slate-200 px-4 py-4 text-slate-700 hover:border-blue-200 hover:bg-blue-50 transition">
                    ...
                    <a href="{{ route('profile') }}" class="flex items-center gap-3 rounded-2xl border border-slate-200 px-4 py-4 text-slate-700 hover:border-blue-200 hover:bg-blue-50 transition">
