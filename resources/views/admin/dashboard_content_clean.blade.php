<div class="space-y-8">
    <div class="rounded-[2rem] bg-white border border-slate-100 shadow-xl shadow-slate-200/60 p-8 sm:p-10">
        <div class="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-8">
            ...
            <div class="flex flex-wrap gap-3">
                <a href="{{ route('admin.books.index') }}" class="inline-flex items-center gap-2 rounded-2xl bg-slate-900 px-5 py-3.5 text-sm font-semibold text-white transition hover:bg-slate-800">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"></path>
                    </svg>
                    Lihat Data Buku
                </a>
                <a href="{{ route('admin.books.create') }}" class="inline-flex items-center gap-2 rounded-2xl border border-slate-200 bg-white px-5 py-3.5 text-sm font-semibold text-slate-700 transition hover:border-blue-200 hover:bg-blue-50 hover:text-blue-700">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5">
        ...
        <div class="rounded-3xl bg-white p-6 border border-slate-100 shadow-lg shadow-slate-200/60">
            ...
            <div class="mt-3 flex items-end justify-between">
                <span class="text-3xl font-black text-slate-900">{{ number_format($stats['memberCount'] ?? 0) }}</span>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
        <div class="xl:col-span-2 rounded-[2rem] bg-white border border-slate-100 shadow-xl shadow-slate-200/60 p-6 sm:p-8">
            <div class="flex items-center justify-between gap-4 mb-6">
                ...
                <a href="{{ route('admin.books.index') }}" class="text-sm font-semibold text-blue-700 hover:text-blue-800">Kelola katalog</a>
            </div>
                        ...
                        @forelse ($recentBooks as $book)
                            <tr class="hover:bg-slate-50/80 transition">
                                ...
                                <td class="px-5 py-4 text-center">
                                    <span class="inline-flex items-center justify-center rounded-full px-3 py-1 text-xs font-bold {{ $book->stok > 0 ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700' }}">
                                        {{ $book->stok }}
                                    </span>
                                </td>
                            </tr>
