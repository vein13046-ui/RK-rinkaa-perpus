...
<?php $__env->startSection('page-description', 'Lihat status peminjaman, ambil buku, dan ajukan pengembalian.'); ?>
    ...
    <div class="space-y-6">
        ...
        <div class="rounded-[2rem] bg-white border border-slate-100 shadow-xl shadow-slate-200/60 p-6 sm:p-8">
            <div class="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6">
                ...
                <a href="<?php echo e(route('user.books')); ?>" class="inline-flex items-center gap-2 rounded-2xl bg-slate-900 px-5 py-3.5 text-sm font-semibold text-white transition hover:bg-slate-800">
                ...
                <tbody class="divide-y divide-slate-100 bg-white">
                    <?php $__empty_1 = true; $__currentLoopData = $borrows; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $borrow): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="hover:bg-slate-50/80 transition">
                            ...
                            <td class="px-5 py-4 text-slate-600"><?php echo e($borrow->borrower_name); ?></td>
                            ...
                            <td class="px-5 py-4 text-center">
                                ...
                                <?php if($borrow->status === 'approved' && $borrow->pickup_deadline): ?>
                                    <div class="mt-2 text-xs font-semibold text-amber-700">
                                        Segera ambil sebelum <?php echo e($borrow->pickup_deadline->translatedFormat('d M Y H:i')); ?>

                                    </div>
                                <?php endif; ?>
                                <?php if($borrow->status === 'picked_up' && $borrow->due_date): ?>
                                    <div class="mt-2 text-xs font-semibold text-slate-500">
                                        Jatuh tempo: <?php echo e($borrow->due_date->translatedFormat('d M Y')); ?>
                                ...
                                <div class="flex flex-wrap items-center justify-center gap-2">
                                    <?php if($borrow->status === 'approved'): ?>
                                        <button
                                            ...
                                            data-action="<?php echo e(route('borrow.pickup', $borrow)); ?>"
                                            data-title="<?php echo e($borrow->book->judul ?? ''); ?>"
                                            data-author="<?php echo e($borrow->book->penulis ?? ''); ?>"
                                            data-category="<?php echo e($borrow->book->kategori ?? ''); ?>"
                                            data-cover="<?php echo e($borrow->book && $borrow->book->cover ? Storage::url($borrow->book->cover) : ''); ?>"
                                            data-fetch-url="<?php echo e(route('borrow.pickup.data', $borrow)); ?>"
                                            data-code="<?php echo e($borrow->pickup_code); ?>"
                                            data-deadline="<?php echo e(optional($borrow->pickup_deadline)->translatedFormat('d M Y H:i')); ?>"
                                        >
                                            Ambil Buku
                                        </button>
                                    <?php elseif($borrow->status === 'picked_up'): ?>
                                        <form action="<?php echo e(route('borrow.request-return', $borrow)); ?>" method="POST">
                                            <?php echo csrf_field(); ?>
...
<?php echo $__env->make('layouts.panel', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp8882\htdocs\perpustakaan\resources\views/borrow/status_peminjaman_user_clean.blade.php ENDPATH**/ ?>